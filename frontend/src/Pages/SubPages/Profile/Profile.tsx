          ...
          <div className="px-5 pb-5">
            ...
            <div className="flex items-center flex-wrap gap-3 mt-2">
              ...
              {(me?.city || me?.state) && (
                <div className="flex items-center gap-1 text-xs text-[#3a3a52]">
                  <MapPin size={11} />
                  <span>{[me?.city, me?.state].filter(Boolean).join(", ")}</span>
                </div>
              )}
              ...
              {me?.dob && (
                <div className="flex items-center gap-1 text-xs text-[#3a3a52]">
                  <Calendar size={11} />
                  <span>{new Date(me.dob).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-6 mt-4 pt-4 border-t border-[#2a2a38]">
              <div className="text-center">
                <p className="text-sm font-bold text-[#e0e0f0]">{posts.length}</p>
                <p className="text-xs text-[#3a3a52]">Posts</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-[#e0e0f0]">{me?.myCommunities?.length ?? 0}</p>
        ...
        <div>
          ...
          {loadingPosts ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-2xl border border-[#2a2a38] bg-[#13131a]">
              ...
              <p className="text-xs text-[#3a3a52]">Your posts will appear here</p>
            </div>
          ) : (

...
const SideBar = ({ user }: SideBarProps) => {
  ...
  const communities: Community[] = (user?.myCommunities ?? [])
    .map((item) =>
                  ...
                  >
                    <div className="w-6 h-6 rounded-md overflow-hidden shrink-0">
                      {community.image ? (
                        ...
                        <div
                          className={`w-full h-full bg-linear-to-br ${getAccent(community.name)} flex items-center justify-center text-[10px] font-bold text-white`}
                        >
